...
from ai.catalog import CatalogItemList
...
class DistributedMailboxAI(DistributedObjectAI):
    ...
    def avatarEnter(self):
        ...
        if not av:
            ...
            self.notify.warning('av %s not in doTable tried to pick up mailbox' % (avId))
            return
        ...
        if self.house.avId == avId:
            ...
            if len(av.mailboxContents) != 0:
                ...
                self.sendUpdate('setMovie', [MAILBOX_MOVIE_WAITING, avId])
                self.sendClearMovie()
            else:
    ...
    def sendExitMovie(self):
        ...
        self.sendUpdate('setMovie', [MAILBOX_MOVIE_EXIT,
                                     self.busy])
    ...
    def sendClearMovie(self):
        ...
        self.busy = 0
        self.av = None
        self.sendUpdate('setMovie', [MAILBOX_MOVIE_CLEAR, 0])
