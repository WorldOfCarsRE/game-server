...
class QuestManagerAI:
    ...
    QuestCheat = config['AI.QuestCheat']
        ...
        if (self.needsQuest(av) and npc.getGivesQuests()):
            ...
            if quests:
                if (Quests.getNumChoices(av.getRewardTier()) == 0):
                    ...
                    npc.assignQuest(av.doId, *quests[0])
                else:
                    ...
                    npc.presentQuestChoice(av.doId, quests)
                return
            else:
    ...
    def rejectAvatar(self, av, npc):
        self.notify.debug("rejecting avatar: avId: %s" % (av.doId))
        npc.rejectAvatar(av.doId)
        return

    def rejectAvatarTierNotDone(self, av, npc):
        self.notify.debug("rejecting avatar because tier not done: avId: %s" % (av.doId))
        npc.rejectAvatarTierNotDone(av.doId)
        return
    ...
    def completeQuest(self, av, npc, questId):
        ...
        if questClass == Quests.TrackChoiceQuest:
            ...
            npc.presentTrackChoice(av.doId, questId, tracks)
        ...
        if nextQuestId == Quests.NA:
            ...
            npc.completeQuest(av.doId, questId, rewardId)
            # Bump the reward
            ...
            if npc.getHq():
                ...
                fromNpcId = npc.getNpcId()
            self.assignQuest(av.doId, fromNpcId, nextQuestId, nextRewardId, nextToNpcId, startingQuest = 0)
            npc.assignQuest(av.doId, nextQuestId, nextRewardId, nextToNpcId)
            eventLogMessage += "|next %s" % (nextQuestId)

        self.air.writeServerEvent('questComplete', av.doId, eventLogMessage)

    def incompleteQuest(self, av, npc, questId, completeStatus, toNpcId):
        self.notify.debug("incompleteQuest: avId: %s questId: %s" %
                          (av.doId, questId))
        npc.incompleteQuest(av.doId, questId, completeStatus, toNpcId)
        return
