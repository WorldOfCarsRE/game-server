...
function retrieveAccount(data)
    ...
    if error_message then
        print(string.format("APIDatabase: retrieveAccount returned an error! \"%s\"", error_message))
        return "{}"
    end
    if response.status_code ~= 200 then
        print(string.format("APIDatabase: retrieveAccount returned %d!, \"%s\"", response.status_code, response.body))
        return "{}"
    end
    return response.body
end

function retrieveCar(participant, data)
    ...
    while (connAttempts < 3) do
        ...
        if error_message then
            print(string.format("APIDatabase: retrieveCar returned an error! \"%s\"", error_message))
            connAttempts = connAttempts + 1
            goto retry
        end

        if response.status_code ~= 200 then
            print(string.format("APIDatabase: retrieveCar returned %d!, \"%s\"", response.status_code, response.body))
            connAttempts = connAttempts + 1
            goto retry
        end
...
function setCarData(participant, playToken, data)
    ...
    while (connAttempts < 3) do
        ...
        if error_message then
            print(string.format("APIDatabase: setCarData returned an error! \"%s\"", error_message))
            connAttempts = connAttempts + 1
            goto retry
        end

        if response.status_code ~= 200 then
            print(string.format("APIDatabase: setCarData returned %d!, \"%s\"", response.status_code, response.body))
            connAttempts = connAttempts + 1
            goto retry
        end
...
function setCarFields(participant, playToken, data)
    local request = {playToken = playToken, fieldData = data}
    local json = require("json")
    local result, err = json.encode(request)
    ...
    while (connAttempts < 3) do
        ...
        if error_message then
            print(string.format("APIDatabase: setCarFields returned an error! \"%s\"", error_message))
            connAttempts = connAttempts + 1
            goto retry
        end

        if response.status_code ~= 200 then
            print(string.format("APIDatabase: setCarFields returned %d!, \"%s\"", response.status_code, response.body))
            connAttempts = connAttempts + 1
            goto retry
        end
