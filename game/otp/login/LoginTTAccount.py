...
class LoginTTAccount(LoginBase.LoginBase, TTAccount.TTAccount):
    ...
    def authenticateParentPassword(self, loginName, password, parentPassword):
        if base.cr.withParentAccount:
            ...
            try:
                errorMsg = self.talk('authenticateParentUsernameAndPassword',
        ...
        if self.useTTSpecificLogin:
            ...
            try:
                errorMsg = self.talk('authenticateParentPasswordNewStyle',
                                     data=self.makeLoginDict(
                                         loginName, parentPassword))
                if not errorMsg:
                    return (1, None)
    ...
    def authenticateDelete(self, loginName, password):
        if self.useTTSpecificLogin:
            ...
            try:
                errorMsg = self.talk('authenticateDeleteNewStyle',
                                     data=self.makeLoginDict(
                                         loginName, password))
                if not errorMsg:
                    return (1, None)
