...
from game.otp.login import SecretFriendsInfoPanel
from game.otp.login import PrivacyPolicyPanel
from game.otp.otpbase import OTPLocalizer
from direct.directnotify import DirectNotifyGlobal
from game.otp.login import LeaveToPayDialog
...
class ChatManager(DirectObject.DirectObject):
    ...
    def isObscured(self):
        return (self._ChatManager__normalObscured,
                self._ChatManager__scObscured)
    ...
    def enterWhisper(self, avatarName, avatarId, playerId=None):
        ...
        if (avatarUnderstandable or
                playerUnderstandable) and online and not normalButtonObscured:
        ...
        if online:
            ...
            self.changeFrameText(OTPLocalizer.ChatManagerWhisperToName %
                                 chatName)
        else:
            self.whisperScButton['state'] = 'inactive'
            self.changeFrameText(OTPLocalizer.ChatManagerWhisperOffline %
                                 chatName)
    ...
    def enterChatMoreInfo(self):
        ...
        self.accept('secretFriendsInfoDone',
                    self._ChatManager__secretFriendsInfoDone)
