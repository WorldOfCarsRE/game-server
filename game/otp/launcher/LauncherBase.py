...
class LauncherBase(DirectObject):
    ...
    BANDWIDTH_ARRAY = [
        1800, 3600, 4200, 6600, 8000, 12000, 16000, 24000, 32000, 48000, 72000,
        96000, 128000, 192000, 250000, 500000, 750000, 1000000, 1250000,
        1500000, 1750000, 2000000, 3000000, 4000000, 6000000, 8000000,
        10000000, 12000000, 14000000, 16000000, 24000000, 32000000, 48000000,
        64000000, 96000000, 128000000, 256000000, 512000000, 1024000000
    ]
    ...
    def __init__(self):
        ...
        os.environ[
            'CONFIG_CONFIG'] = ':_:configdir_.:configpath_:configname_Configrc.exe:configexe_1:configargs_-stdout ' + Configrc_args
        ...
        if tmpVal is None:
            ...
            self.fromCD = tmpVal
        self.notify.info('patch directory is ' + str(self.fromCD))
        ...
        tmpPhase3Map = [0.001, 0.996, 0.0, 0.0, 0.0030000000000000001]
        phaseIdx = 0
        for phase in self.LauncherPhases:
            percentPhaseCompleteKey = 'PERCENT_PHASE_COMPLETE_' + str(phase)
        ...
        while not connOk:
            ...
            if not connOk:
                self.notify.warning('Could not contact download server at %s' %
                                    testurl.cStr())
    ...
    def getNextDownloadServer(self):
        ...
        self.downloadServer = self.downloadServerList[
            self.nextDownloadServerIndex]
    ...
    def downloadTask(self, task):
        ...
        if self.httpChannel.isValid() and self.httpChannel.isDownloadComplete(
            ...
            del task.callback
            return task.done
        elif statusCode == HTTPChannel.SCDownloadOpenError or statusCode == HTTPChannel.SCDownloadWriteError:
    ...
    def downloadMultifile(self, serverFilename, localFilename, mfname,
        ...
        if currentSize != 0:
            ...
            self.httpChannel.beginGetSubdocument(DocumentSpec(task.mfURL),
    ...
    def downloadPatchSimpleProgress(self, task):
        ...
        percentPatchComplete = int(
            round(
                (bytesWritten / float(totalBytes)) * self.downloadPercentage))
        self.setPercentPhaseComplete(self.currentPhase, percentPatchComplete)

    def getPercentPatchComplete(self, bytesWritten):
        return int(
            round(((self.patchDownloadSoFar + bytesWritten) /
                   float(self.totalPatchDownload)) * self.downloadPercentage))
    ...
    def downloadMultifileWriteToDisk(self, task):
        ...
        if task.totalSize != 0:
            percentComplete = int(
                round((bytesWritten / float(task.totalSize)) *
    ...
    def downloadMultifileTask(self, task):
        ...
        if self.httpChannel.isValid() and self.httpChannel.isDownloadComplete(
            ...
            self.downloadMultifile(task.serverFilename, task.localFilename,
                                   task.mfname, task.callback, task.totalSize,
                                   bytesWritten, task.callbackProgress)
        elif (statusCode == 416
              or statusCode == HTTPChannel.SCDownloadInvalidRange
        ...
        self.handleDownloadFatalError(task)
        self.downloadMultifile(task.serverFilename, task.localFilename,
                               task.mfname, task.callback, task.totalSize, 0,
    ...
    def extract(self, mfname, localFilename, destDir, callback):
        ...
        self.notify.info('Extracting %d subfiles from multifile %s.' %
                         (numFiles, mfname))
    ...
    def patch(self, patchFile, patcheeFile, callback):
        self.notify.info('patch: request: ' + patchFile.cStr() + ' patchee: ' +
                         patcheeFile.cStr())
        self.launcherMessage(
    ...
    def prepareClient(self):
        ...
        if not self.topDir.exists():
            self.notify.info('prepareClient: Creating top directory: ' +
                             self.topDir.cStr())
            os.makedirs(self.topDir.toOsSpecific())

        if not self.dbDir.exists():
            self.notify.info('prepareClient: Creating db directory: ' +
                             self.dbDir.cStr())
            os.makedirs(self.dbDir.toOsSpecific())

        if not self.patchDir.exists():
            self.notify.info('prepareClient: Creating patch directory: ' +
                             self.patchDir.cStr())
            os.makedirs(self.patchDir.toOsSpecific())

        if not self.mfDir.exists():
            self.notify.info('prepareClient: Creating mf directory: ' +
                             self.mfDir.cStr())
    ...
    def downloadClientDbStarterFile(self):
        ...
        localFilename = Filename(self.dbDir,
                                 Filename(self.compClientDbFilename))
    ...
    def updatePhase(self, phase):
        self.notify.info('Updating multifiles in phase: ' + str(phase))
    ...
    def updateNextMultifile(self):
        ...
        self.notify.info('Done updating multifiles in phase: ' +
                         str(self.currentPhase))
        self.progressSoFar += int(
            round(self.phaseOverallMap[self.currentPhase] * 100))
        self.notify.info('progress so far ' + str(self.progressSoFar))
        messenger.send('phaseComplete-' + str(self.currentPhase))
        if nextIndex < len(self.LauncherPhases):
    ...
    def getMultifile(self, mfname):
        ...
        decompressedFilename = Filename(self.mfDir,
                                        Filename(decompressedMfname))
        if (not self.dldb.clientMultifileComplete(mfname)
            ...
            clientVer = self.dldb.getVersion(Filename(decompressedMfname),
                                             clientMd5)
            if clientVer != -1:
                self.notify.info(
        ...
        if not self.dldb.clientMultifileComplete(
                mfname) or not decompressedFilename.exists():
            self.maybeStartGame()
            ...
            if currentSize == 0:
                ...
                self.downloadMultifile(self.contentDir + mfname, localFilename,
                                       mfname, self.downloadMultifileDone,
                                       totalSize, 0,
                ...
                if clientHash.eq(serverHash):
                    ...
                    self.downloadMultifile(self.contentDir + mfname,
                                           localFilename, mfname,
                    ...
                    self.dldb.setClientMultifileIncomplete(self.currentMfname)
                    self.dldb.setClientMultifileSize(self.currentMfname, 0)
                    self.dldb.setClientMultifileHash(self.currentMfname,
                                                     serverHash)
                    self.getMultifile(self.currentMfname)
                else:
                    self.setPandaErrorCode(6)
    ...
    def getDecompressMultifile(self, mfname):
        if not self.DecompressMultifiles:
            ...
            self.notify.info(
                'decompressMultifile: Multifile already decompressed: ' +
                mfname)
            self.decompressMultifileDone()
    ...
    def extractMultifile(self, mfname):
        if not self.dldb.clientMultifileExtracted(mfname):
            self.maybeStartGame()
            self.notify.info('extractMultifile: Extracting multifile: ' +
                             mfname)
            decompressedMfname = os.path.splitext(mfname)[0]
            localFilename = Filename(self.mfDir, Filename(decompressedMfname))
    ...
    def extractMultifileDone(self):
        ...
        self.notify.info(
            'extractMultifileDone: Finished updating multifile: ' +
            self.currentMfname)
        self.patchMultifile()

    def getPatchFilename(self, fname, currentVersion):
        return fname + '.v' + str(currentVersion) + '.' + self.patchExtension

    def downloadPatches(self):
        if len(self.patchList) > 0:
            (self.currentPatch, self.currentPatchee,
             self.currentPatchVersion) = self.patchList.pop()
    ...
    def decompressPatchDone(self):
        self.notify.info('decompressPatchDone: Patching file: ' +
                         self.currentPatchee + ' from ver: ' +
                         str(self.currentPatchVersion))
    ...
    def reextractNextFile(self):
        ...
        while not failure and len(self.reextractList) > 0:
            ...
            if subfileIndex >= 0:
                destFilename = Filename(self.topDir,
                                        Filename(currentReextractFile))
    ...
    def patchMultifile(self):
        ...
        self.notify.info(
            'patchMultifile: Checking for patches on multifile: ' +
            self.currentMfname)
        self.patchList = []
        clientMd5 = HashVal()
        decompressedMfname = os.path.splitext(self.currentMfname)[0]
        ...
        if clientVer == 1:
            ...
            self.notify.info('patchMultifile: Invalid hash for file: ' +
                             self.currentMfname)
            self.maybeStartGame()
            if self.curMultifileRetry < self.multifileRetries:
                self.notify.info(
    ...
    def patchAndHashTask(self, task):
        ...
        if self.VerifyFiles and self.dldb.hasVersion(Filename(fname)):
            ...
            if clientVer == 1:
                ...
                self.notify.info('patchAndHash: Invalid hash for file: ' +
                                 fname)
                self.reextractList.append(fname)
                self.PAHClean = 0
    ...
    def setPercentPhaseComplete(self, phase, percent):
        ...
        if oldPercent != percent:
            ...
            messenger.send(
                ...
                [phase, percent,
                 self.getBandwidth(), self.byteRate])
            percentPhaseCompleteKey = 'PERCENT_PHASE_COMPLETE_' + str(phase)
            self.setRegistry(percentPhaseCompleteKey, percent)
            self.overallComplete = int(
                round(percent *
                      self.phaseOverallMap[phase])) + self.progressSoFar
    ...
    def getBandwidth(self):
        if self.backgrounded:
            bandwidth = self.BANDWIDTH_ARRAY[
                self.bandwidthIndex] - self.TELEMETRY_BANDWIDTH
        else:
            bandwidth = self.BANDWIDTH_ARRAY[self.bandwidthIndex]
        if self.MAX_BANDWIDTH > 0:
            bandwidth = min(bandwidth, self.MAX_BANDWIDTH)
