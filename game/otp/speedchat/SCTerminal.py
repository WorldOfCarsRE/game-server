...
class SCTerminal(SCElement):
    ...
    def privSetSettingsRef(self, settingsRef):
        ...
        if self.settingsRef is not None:
            pass
        self._handleWhisperModeSV.set(not self.isWhisperable())
    ...
    def isDisabled(self):
        if self._SCTerminal__disabled and self.isWhispering():
            pass
        return not self.isWhisperable()
