...
from game.otp.avatar import PlayerBase
from game.otp.distributed.ClsendTracker import ClsendTracker
...
class DistributedPlayerAI(DistributedAvatarAI.DistributedAvatarAI, PlayerBase.PlayerBase, ClsendTracker):
    ...
    def announceGenerate(self):
        ...
        if self.announceArrival:
            self._doPlayerEnter()

    def _announceArrival(self, task):
    ...
    def _doPlayerEnter(self):
        ...
        # HACK: We delay the announcement of our arrival by a second to resolve a issue with high latency connections.
        enterFudgeTime = simbase.config.GetFloat('district-enter-fudge-time', 0.0)
        if enterFudgeTime > 0.0:
            taskMgr.doMethodLater(enterFudgeTime, self._announceArrival, '_announceArrival')
        else:
